...
# Relay implementation notes (004-modernize-merchant-and)
# THIS FILE IS THE ONLY VISUAL SOURCE OF TRUTH for the Relay SPA.
# Scope: entire SPA including retractable sidebar + top bar + all pages.
# Do not invent colors, gradients, or component looks outside this document.
# Font substitutes (Coinbase Display/Sans licensed — not bundled):
#   Display → Inter weight 400 (letter-spacing from typography scale below)
#   Sans/UI → Inter
#   Mono/number-display → IBM Plex Mono
# Primary brand voltage: Coinbase Blue #0052ff (not ERP #0D3A69 / #E394AF).
